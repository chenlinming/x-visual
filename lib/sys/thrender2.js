...
import { EffectComposer } from '../../packages/three/postprocessing/EffectComposer.js';
import { RenderPass } from '../../packages/three/postprocessing/RenderPass.js';
import { SMAAPass } from '../../packages/three/postprocessing/SMAAPass.js';
import {SSAARenderPass}  from "../../packages/three/postprocessing/SSAARenderPass.js"
import { vs,fs } from './fxaaShader.js';
import {ShaderPass} from "../../packages/three/postprocessing/ShaderPass.js";
import {convertVertexShaderToWebGL2,convertFragmentShaderToWebGL2} from "../../packages/three/postprocessing/GlslUtil.js";
import * as kernel from "./kernels"
	...
	init(canvas, x) {
		...
		this.mrt = new THREE.WebGLMultiRenderTarget(
			...
			THREE.WebGLProgram.mrt_num,
			{
				...
				minFilter: THREE.LinearMipmapLinearFilter,//THREE.LinearFilter
		...
		let {vertexShader, fragmentShader} = xglsl.xvShader(ShaderFlag._filters,
			x.options, undefined, !!(x.options.finalQuad && x.options.finalQuad.debug));

		let filterUniforms = Object.assign( {
				u_blurRadius: x.options.finalQuad
					&& x.options.finalQuad.blurRadius !== undefined
					? x.options.finalQuad.blurRadius : 4,
				u_blurIntense: x.options.finalQuad
					&& x.options.finalQuad.blurIntense !== undefined
					? x.options.finalQuad.blurIntense : 1.,
			  },
		...
		let copyShader = {
			...
			in vec2 vUv;
			void main() {
				vec3 texel = texture( tDiffuse, vUv ).rgb;
				pc_FragColor  = vec4(texel,opacity);
				
			}`,
			uniforms:{
				tDiffuse:{value:this.mrt.textures[ 0 ]},
				opacity:{value:1.0}
			}
		}
		...
		//this.finalQuad.material.uniforms.tDiffuse.value = this.filterBuffer.textures[ 1 ];
		this.postScene.add( this.finalQuad );
		
		let renderPass = new RenderPass(this.postScene,this.postCamera);
		...
		//let smaaPass = new SMAAPass( glConfig.xfilter.w, glConfig.xfilter.h );
		//this.composer.addPass(smaaPass);
		let beforeCompile = (shader,render) => {
		let vs = convertVertexShaderToWebGL2(shader.vertexShader);
		let fs = convertFragmentShaderToWebGL2(shader.fragmentShader);
    	...
    	let fxaaPass = new ShaderPass( shaderMaterial );
		this.composer.addPass(fxaaPass);
		//this.composer.addPass(blurPass);
		//let ssaaPass = new SSAARenderPass(this.postScene,this.postCamera);
		//let ssaaPass = new SSAARenderPass(scene,this.camera);
		...
		let fs = convertFragmentShaderToWebGL2(shader.fragmentShader);
		 shader.vertexShader = vs;
	     shader.fragmentShader  = fs;}
		let fxaaShader = new THREE.RawShaderMaterial({
		...
		fxaaPass.material.uniforms[ 'resolution' ].value.x = 1 / ( 1024 );
		fxaaPass.material.uniforms[ 'resolution' ].value.y = 1 / ( 1024 );
		*/
		...
		let fxaaShader = new THREE.RawShaderMaterial({
				vertexShader: copyShader2.vertexShader,
				fragmentShader: copyShader2.fragmentShader,
				uniforms:copyShader2.uniforms
			})
		let fxaaPass = new ShaderPass( fxaaShader );*/
		//this.composer.addPass(fxaaPass);
		return x;
