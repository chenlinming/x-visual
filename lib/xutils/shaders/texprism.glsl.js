	...
	return {
		fragmentShader: (`${glx.mrt.f_layout}
			...
			vec4 texY(vec3 e, vec3 P, vec3 c0, vec2 size, vec2 tiles, vec2 boader) {
				vec4 p0d = rayPlaneInsec( e, normalize(P - e), c0, vec3(0., 1., 0.) );
				if (p0d.w > 0.) {
					return texture( u_tex0, (.5 - (p0d.xz - c0.xz) / size) );
				}
				else return vec4(0.);
			}
			...
			void main() {
				...
				+`
				if (vxzWeight <= 0.9) {
				    pc_FragColor = mix( pc_FragColor, texture(u_tex1, fractuv(vUv)),
										${sideAlpha} );
				}

				pc_FragColor.xyz *= u_lightIntensity;
				${!receiveShadow ? '' : 'pc_FragColor.xyz *= shadow();'}
				xColor = pc_FragColor;
				pc_FragColor.xyz = reinhardTone (pc_FragColor.xyz, 1.);
